...
###########################
#
# Load files into a list format
#
setwd("~/modules/data_wrangling")
data=list(
    dge_results=read.table('data/differential_results.csv', sep=',', header=T),
    expr=read.table('data/gene_expression.tsv', header=T, row.names=1),  # first column being the rowname
    annot=read.delim('data/gene_annotations.tsv')
)
...
# define find mean function with inputs
# the function is defined, but not run
...
find_mean_results=subset(merge_results, mean>0, select=c("name", "chrom", "mean"))
head(find_mean_results)

# write out the results
library(writexl)
write_xlsx(find_mean_results, "outputs/find_mean_results.xlsx")
